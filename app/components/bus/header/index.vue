<template>
  <nav
    :class="[
      ...
      isBusRoutesPage && appStore.isListView ? 'bg-transparent' : 'relative',
    ]"
  >
    <div class="w-full flex justify-between items-center">
      ...
      <h1
        :class="
          isBusRoutesPage && appStore.isListView
            ? 'font-bold drop-shadow-2xl tracking-wide text-black'
            : 'font-semibold'
        "
        style="text-shadow: 0 2px 8px rgba(0, 0, 0, 0.6)"
      >
        Графіки міського транспорту Старокостянтинова
      </h1>
      <div class="flex items-center gap-4">
        <!-- Кнопка перемикання вигляду з тултіпом -->
        <UTooltip
          v-if="isBusRoutesPage"
        ...
        <BusHeaderButtons @toggle-search="toggleSearch" />
      </div>
    </div>
    <div class="ml-auto">
...
const buttonClasses = computed(() => {
  ...
  if (isBusRoutesPage.value && appStore.isListView) {
